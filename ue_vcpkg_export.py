...
class UeVcpkgExport:
    def __init__(self, package: str, vcpkg_dir: str, packages_dict: dict, logger_obj=None):
    ...
    def _walk_dependencies(self):
        ...
        for depend_line in depend_info_lines:
            ...
            if dependency_package in self._packages_dict:
                ...
                exporter = UeVcpkgExport(dependency_package, self._vcpkg_dir, self._packages_dict, self._logger)
    ...
    @staticmethod
    def _generate_public_definitions(package_name: str):
        text = f"\"WITH_{package_name.upper()}\""
        return [text]
    ...
    @staticmethod
    def _generate_module_name(package_name: str):
    ...
    def _generate_triplet_file_record(self, triplet: str, triplet_files: list[str]):
        defs = UeVcpkgExport._generate_public_definitions(self._package)
    ...
    def _generate_build_cs(self, module_build_cs_dir: str, triplet_files: dict[str, list[str]]):
        ...
        for triplet, triplet_files in triplet_files.items():
            triplet_file_record = self._generate_triplet_file_record(triplet, triplet_files)
        ...
        build_cs = module_build_template.substitute(
            {
                "ModuleName": UeVcpkgExport._generate_module_name(self._package),
    ...
    def _generate_module_loader_h(self, module_loader_h_dir: str, triplet_files: dict[str, list[str]]):
        ...
        module_name = UeVcpkgExport._generate_module_name(self._package)
    ...
    def _generate_module_loader_cpp(self, module_loader_cpp_dir: str):
        ...
        module_name = UeVcpkgExport._generate_module_name(self._package)
        text = module_loader_cpp_template.substitute({"ModuleName": module_name})
        return text
    ...
    def export(
        ...
        module_name = UeVcpkgExport._generate_module_name(self._package)
        module_loader_name = UeVcpkgExport._generate_module_name(self._package) + "Loader"
        ...
        for triplet in triplets:
            owned_files = self.get_owned_files(triplet, fnmatch_patterns, extensions_whitelist)
...
def main():
    ...
    parser.add_argument("--package_blacklist", default="package-blacklist.txt")
    ...
    package_blacklist_path = args.package_blacklist
    package_blacklist = []
    if package_blacklist_path is not None:
        with open(package_blacklist_path, "r") as package_blacklist_file:
            package_blacklist = [line.strip() for line in package_blacklist_file.readlines()]

    packages_dict = {}
    export_obj = UeVcpkgExport(args.target_package, args.vcpkg_dir, packages_dict, logger)
    ...
    if not args.with_dependencies:
        ...
        for package_name, package_export in packages_dict.items():
            ...
            for package_blacklist_entry in package_blacklist:
                if re.match(package_blacklist_entry, package_name):
                    logger.warning(f"Package {package_name} blacklisted.")
                    blacklisted = True
                    break
